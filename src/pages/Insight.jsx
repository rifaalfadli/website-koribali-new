            ...
            <div className="mb-16">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
                ...
                <div className="lg:col-span-7 xl:col-span-8">
                  <Link to={`/insight/${mainTrending.id}`} className="block group h-full bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden hover:-translate-y-1 hover:shadow-2xl hover:shadow-blue-900/10 transition-all duration-300">
                    ...
                    <div className="p-8">
                      <div className="flex items-center space-x-4 mb-4 text-sm font-semibold">
                        ...
                        <span className="bg-slate-950 text-blue-400 px-3 py-1 rounded-full border border-slate-800">{mainTrending.category}</span>
                      </div>
                      <h3 className="text-2xl md:text-3xl font-bold text-white mb-4 group-hover:text-blue-400 transition-colors leading-snug">
                        {mainTrending.title}
                      </h3>
                ...
                <div className="lg:col-span-5 xl:col-span-4 flex flex-col gap-4">
                  {sideTrending.map((article, idx) => (
                    <Link to={`/insight/${article.id}`} key={article.id} className="block group bg-slate-900 border border-slate-800 rounded-2xl p-6 hover:-translate-x-1 hover:border-blue-900/50 transition-all duration-300 flex-1 flex flex-col justify-center">
                      ...
                      <h4 className="text-lg font-bold text-white mb-3 group-hover:text-blue-400 transition-colors leading-snug line-clamp-2">
                        {article.title}
                      </h4>
                      <div className="flex items-center text-xs text-slate-500 mt-auto">
                        ...
                        <span>•</span>
                        <span className="ml-2 group-hover:text-blue-400 transition-colors">Baca selengkapnya</span>
                      </div>
                    </Link>
                  ))}
                </div>
                ...
                {visibleArticles.map((article) => (
                  <Link to={`/insight/${article.id}`} key={article.id} className="group flex flex-col h-full bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden hover:-translate-y-1 hover:shadow-xl transition-all duration-300">
                    ...
                    <div className="p-6 md:p-8 flex flex-col flex-grow">
                      ...
                      <h3 className="text-xl font-bold text-white mb-4 group-hover:text-blue-400 transition-colors leading-snug line-clamp-2">
                        {article.title}
                      </h3>
                      ...
                      <div className="mt-auto flex items-center text-blue-500 text-sm font-bold group-hover:text-blue-400 transition-colors border-t border-slate-800/80 pt-5">
                        Read More
