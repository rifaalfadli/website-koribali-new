    ...
    <>
      ...
      <div className="pt-10 bg-white dark:bg-slate-950">
        <SectionWrapper className="pt-0">
          ...
          {currentPage === 1 && (
            <div className="mb-16">
              <h2 className="text-3xl font-bold text-slate-900 dark:text-white font-display mb-8">Trending</h2>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">

                {/* Main Trending (Kiri) */}
                <div className="lg:col-span-7 xl:col-span-8">
                  <Link to={`/insight/${mainTrending.id}`} className="block group h-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden hover:-translate-y-1 hover:shadow-2xl hover:shadow-blue-100/50 dark:hover:shadow-blue-900/10 transition-all duration-300">
                    ...
                    <div className="p-8">
                      <div className="flex items-center space-x-4 mb-4 text-sm font-semibold">
                        <span className="text-slate-500 dark:text-slate-400">{formatDate(mainTrending.date)}</span>
                        <span className="bg-slate-100 dark:bg-slate-950 text-blue-500 dark:text-blue-400 px-3 py-1 rounded-full border border-slate-200 dark:border-slate-800">{mainTrending.category}</span>
                      </div>
                      <h3 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-4 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors leading-snug">
                        {mainTrending.title}
                      </h3>
                      <p className="text-slate-500 dark:text-slate-400 text-lg leading-relaxed mb-8 line-clamp-2">
                        {mainTrending.excerpt}
                      </p>
                      <div className="flex items-center text-blue-500 font-bold group-hover:text-blue-400">
                ...
                <div className="lg:col-span-5 xl:col-span-4 flex flex-col gap-4">
                  {sideTrending.map((article, idx) => (
                    <Link to={`/insight/${article.id}`} key={article.id} className="block group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 hover:-translate-x-1 hover:border-blue-300 dark:hover:border-blue-900/50 transition-all duration-300 flex-1 flex flex-col justify-center">
                      <div className="flex items-center space-x-3 mb-3 text-xs font-semibold">
                        <span className="bg-slate-100 dark:bg-slate-950 text-slate-600 dark:text-slate-300 px-2.5 py-1 rounded-md border border-slate-200 dark:border-slate-800">#{idx + 2}</span>
                        <span className="text-blue-500">{article.category}</span>
                        <span className="text-slate-500">{formatDate(article.date)}</span>
                      </div>
                      <h4 className="text-lg font-bold text-slate-900 dark:text-white mb-3 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors leading-snug line-clamp-2">
                        {article.title}
                      </h4>
                      <div className="flex items-center text-xs text-slate-500 mt-auto">
                        ...
                        <span>•</span>
                        <span className="ml-2 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors">Baca selengkapnya</span>
                      </div>
                    </Link>
                  ))}
                </div>
          ...
          <div className="mb-12">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white font-display mb-8">
              {activeCategory === 'Semua' ? 'Blog Lainnya' : `Kategori: ${activeCategory}`}
            </h2>

            {visibleArticles.length === 0 ? (
              <div className="text-center py-20 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 border-dashed rounded-3xl text-slate-500">
                ...
                {visibleArticles.map((article) => (
                  <Link to={`/insight/${article.id}`} key={article.id} className="group flex flex-col h-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] overflow-hidden hover:-translate-y-1 transition-all duration-300">
                    ...
                    <div className="p-6 md:p-8 flex flex-col flex-grow">
                      ...
                      <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-4 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors leading-snug line-clamp-2">
                        {article.title}
                      </h3>
                      ...
                      <div className="mt-auto flex items-center text-blue-500 text-sm font-bold group-hover:text-blue-400 transition-colors border-t border-slate-200 dark:border-slate-800/80 pt-5">
                        Read More
