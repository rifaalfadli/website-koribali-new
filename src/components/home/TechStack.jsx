...
const TechStack = () => {
    ...
    return (
        <section className="max-w-6xl mx-auto bg-slate-950 py-20 overflow-hidden border-t border-slate-900/50">
            <div className="px-4 md:px-8 mb-12 text-center">
                ...
                <p className="text-slate-400 mt-4 max-w-3xl mx-auto text-sm md:text-base">
                    Kami menggunakan teknologi standar industri terkini untuk memastikan setiap perancangan sipil dan sistem aplikasi digital Anda berjalan stabil, aman, dan dapat diskalakan.
                </p>
            </div>
            ...
            <div className="relative overflow-hidden mb-6 py-2" style={fadeStyle}>
                ...
                <div className="flex gap-4 sm:gap-6 animate-scroll-left w-max hover:[animation-play-state:paused]">
                    {[...techRow1, ...techRow1].map((tech, index) => (
                        <div
                            key={`row1-${index}`}
                            className="flex items-center gap-4 bg-slate-900 border border-slate-800 rounded-2xl p-4 min-w-[260px] md:min-w-[280px] hover:border-red-900/50 hover:shadow-lg hover:shadow-red-900/10 transition-all duration-300 group cursor-default"
                        >
                            <div className="w-12 h-12 flex-shrink-0 flex items-center justify-center bg-slate-950 rounded-xl border border-slate-800/50 p-2 group-hover:border-red-900/30 transition-colors">
                                <img src={tech.img} alt={tech.name} className="w-full h-full object-contain" />
                            </div>
                            <div className="flex flex-col text-left">
                                <span className="text-white font-bold text-sm md:text-base font-display tracking-tight group-hover:text-red-400 transition-colors">{tech.name}</span>
                                <span className="text-slate-400 text-xs md:text-[13px]">{tech.role}</span>
                            </div>
                        </div>
                    ))}
                </div>
