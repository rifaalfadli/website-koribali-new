      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {visibleProjects.map((project) => (
          <Link to={`/project/${project.id}`} key={project.id} className="group cursor-pointer block h-full">
            <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1.5rem] overflow-hidden transition-all duration-300 h-full flex flex-col hover:-translate-y-1">
              ...
              <div className="aspect-[4/3] bg-slate-100 dark:bg-slate-950 relative overflow-hidden flex-shrink-0">
                ...
                <div className="absolute top-4 left-4">
                  <span className="bg-white/80 dark:bg-black/60 backdrop-blur-md text-blue-500 dark:text-blue-400 text-xs font-semibold px-3.5 py-1.5 rounded-full border border-blue-200 dark:border-blue-900/40 uppercase tracking-wide">
                    {project.category}
                  </span>
                </div>
              </div>
